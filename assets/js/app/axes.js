class Axes {
  ...
  callDrag() {
    ...
    let dragAxisPoint = d3.drag()
      ...
      .on('drag end', () => {
        ...
        this.setPoint( coords, this.axisPoint );
        this.setScale( this.pointsVal[ this.axisPoint ], this.axisPoint );
        this.render();
        this.app.update();
      });
  ...
  setOrigins( coords ) {
    ...
    this.lengths.y = this.app.sizes.height - this.origin.y;
  ...
  scalePoint( coord ) {
    return [
      this.scale.x( coord.x ) + this.origin.x,
      this.scale.y( coord.y ) + this.origin.y
    ]
  }

  invertPoint( coord ) {
    return {
      x: this.scale.x.invert( coord[0] - this.origin.x ),
      y: this.scale.y.invert( coord[1] - this.origin.y )
    }
  }
